...
class PhysicsInformedNN:
    ...
    def __init__(self, t_train, X_train, T_trian, H_trian, U0, t_f, lb, ub, layers, layers_alpha, sf,alpha_lb,alpha_ub):
    ...
    def net_alpha(self, t, T, H):
        ...
        bound_alpha = [tf.constant(self.alpha_lb, dtype=tf.float64), tf.constant(self.alpha_ub, dtype=tf.float64)]
    ...
    def net_f(self, t, T, H):
        ...
        f_X = X_t - (30 - alpha) * X + (0.0008)* (X ** 2) * self.sf
        return f_X
...
if __name__ == "__main__":
 ...
 #   X024only = X_star24only[0]
  ...
  #  X024 =  X024 * sf
    ...
    for j in range(5):
        ...
        if j==0:
         ...
         alpha_ub = 1
            ...
            alpha_lb = min(alpha_min, 0 + j*0.05)
            alpha_ub = max(alpha_max, 1-j*0.05)
        alpha_lb_records.append(alpha_lb)
        alpha_ub_records.append(alpha_ub)
        model = PhysicsInformedNN(t_train, X_train, T_trian, H_trian, U0, t_f, lb, ub,layers, layers_alpha, sf, alpha_lb, alpha_ub)
        ####Training
        LBFGS = True
        # LBFGS = False
        ...
        alpha_min = min(alpha)
        alpha_max = max(alpha)
        alpha_min_records.append(alpha_min)
        alpha_max_records.append(alpha_max)
        import datetime
        ...
        ax.plot(t_star, Alpha, 'r-', lw=4, color=(200 / 255, 100 / 255, 20 / 255), label='alpha--True')
        ax.legend(fontsize=22)
        ax.tick_params(axis='both', labelsize=24)
        ax.ticklabel_format(axis='y', style='sci', scilimits=(3, 3))
        ax.grid(True)
    ...
    # #储存alpha的记录值
    # alphalb = np.asarray(alpha_lb_records)
    # alphaub = np.asarray(alpha_ub_records)
    # alphamin = np.asarray(alpha_min_records)
    # alphamax = np.asarray(alpha_max_records)
    # np.savetxt(save_results_to + 'alphalb.txt', alphalb.reshape((-1, 1)))
    # np.savetxt(save_results_to + 'alphaub.txt', alphaub.reshape((-1, 1)))
    # np.savetxt(save_results_to + 'alphamin.txt', alphamin.reshape((-1, 1)))
    # np.savetxt(save_results_to + 'alphamax.txt', alphamax.reshape((-1, 1)))