...
class PhysicsInformedNN:
    ...
    def net_alpha(self, t,T, H):
        ...
        bound_alpha = [tf.constant(-0.05, dtype=tf.float64), tf.constant(1.95, dtype=tf.float64)]
    ...
    def net_f(self, t, T, H):
        ...
        f_X = X_t - (0.95 - alpha) * X + (1e-4)* (X ** 2) * self.sf
        return f_X
...
if __name__ == "__main__":
    ...
    # Load data
    data_frame = pandas.read_csv('Data/Data_2022.csv')
    data_frame24 = pandas.read_csv('Data/Data_2024.csv')
    ...
    X_star = X_star[2:]
    T_star = data_frame['T'][2:]
    H_star = data_frame['H'][2:]

    # data 2024
    X_star24 = data_frame24['X']  # T x 1 array
    # 3 days average
    X_star24 = X_star24.rolling(window=3).mean()
    X_star24 = X_star24[2:]
    T_star24 = data_frame24['T'][2:]
    H_star24 = data_frame24['H'][2:]
    ...
    for j in range(10):
        ...
        font = 24
        fig, ax = plt.subplots()
        ax.plot(t_star, X / sf, 'r-', lw=2, color=(206 / 255, 170 / 255, 208 / 255), label='PINN')
        ax.plot(t_star, X_star / sf, 'r-', lw=2, color=(97 / 255, 156 / 255, 217 / 255), label='2022')
        ...
        font = 24
        fig, ax = plt.subplots()
        ax.plot(t_star, alpha, 'r-', lw=2, color=(206 / 255, 170 / 255, 208 / 255), label='alpha')
        ax.legend(fontsize=22)
        ax.tick_params(axis='both', labelsize=24)
        ax.ticklabel_format(axis='y', style='sci', scilimits=(3, 3))
        ax.grid(True)
        ...
        font = 24
        fig, ax = plt.subplots()
        ax.plot(t_star, X1, 'k--', marker='o', lw=2, markersize=5, label='Odeslover')
        ax.plot(t_star, X_star / sf, 'r-', lw=2, color=(97 / 255, 156 / 255, 217 / 255), label='Ture data')
        ax.legend(fontsize=22)
        ax.tick_params(axis='both', labelsize=24)
        ...
        font = 24
        fig, ax = plt.subplots()
        ax.plot(t_star, X24, 'k--', marker='o', lw=2, markersize=5, label='Odeslover24')
        ax.plot(t_star, X_star24 / sf, 'r-', lw=2, color=(97 / 255, 156 / 255, 217 / 255), label='Ture data')
        ax.legend(fontsize=22)
        ax.tick_params(axis='both', labelsize=24)
        ax.ticklabel_format(axis='y', style='plain')
        ax.grid(True)
